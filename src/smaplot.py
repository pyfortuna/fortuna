...
def plotSMA(df,smaFilename,w,h):
  ...
  ax.plot_date(df.index.values, df['close'].values, color='r', linestyle='solid', marker=',', linewidth=1, label='Price')
...
def plotMACD(df,macdFilename,w,h):
  fig, ax = plt.subplots(figsize=(w,h))
  ax.plot_date(df.index.values, df['macd'].values, color='b', linestyle='solid', marker=',', linewidth=1, label='MACD')
  ax.legend(frameon=False)
  ax.xaxis_date()
  ax.xaxis.set_major_locator(mdates.MonthLocator(bymonth=[3,6,9,12]))
  ax.xaxis.set_major_formatter(mdates.DateFormatter('%b\n%Y'))
  plt.savefig(macdFilename, dpi=300)
